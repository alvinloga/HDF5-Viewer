...
- Added a release-acceptance reviewer handoff generator that summarizes artifact identity, remaining manual fields, functional rows, visual evidence gaps, and the final validator command without completing DV-1101/DV-1102.
...
- Removed legacy `gui/activity_bar.py` and `gui/secondary_bar.py`; target shell command regions, command registry, and standard state components now own activity/navigation chrome without emoji-based legacy rails.
- Removed legacy `gui/status_bar.py`, `gui/bottom_panel.py`, and `gui/command_palette.py`; target shell status regions, bottom tabs, command registry, localization/accessibility, and state components now own these workbench surfaces without the removed legacy event bus.
- Removed legacy `gui/editor/attr_panel.py`, `data_editor.py`, `data_table.py`, `file_panel.py`, and `tab_manager.py`; target base views, document controller, and editing session contracts now own table/array/text rendering, tab/document lifecycle, and safe-edit state.
- Removed the remaining legacy `gui/` package shell and theme bridge; target GUI ownership now lives only under `data_viewer/gui`.
- Completed DV-1008 legacy runtime removal: root legacy `main.py`, `core/`, `gui/`, `plugins/`, `services/`, `utils/`, `HDF5Viewer.spec`, and obsolete build entrypoints are absent, with the target Data Viewer bootstrap, tests, CI packages, and documentation as the current release surface.
...
- Fixed the release-acceptance packet validator so the documented direct script invocation (`python tools/validate_release_acceptance_packet.py ...`) works outside pytest/module import contexts.
- Fixed the release-acceptance packet hydration helper so saved GitHub Actions artifacts JSON produced by Windows PowerShell redirection is accepted.
...
- Removed legacy runtime imports from target pytest configuration and environment lifecycle checks; `tests/conftest.py` and `tests/test_test_environment.py` now cover only target process stability.
- Removed the empty legacy `utils/` compatibility package; target utility/config/path behavior remains under `data_viewer/` and package deletion is guarded by DV-1008 tests.
- Removed the legacy `services/` package; export/search behavior is now covered by target export queue/exporting/navigation services and DV-1008 package deletion guards.
...
- Removed the obsolete legacy `tests/test_gui_interaction.py` source-import suite; GUI interaction coverage is now guarded by target shell, base-view, dialog, i18n/accessibility, state, theme, navigation/search, and command-registry tests.
