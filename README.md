...
- `core/`、`gui/` 是 legacy/旧版 HDF5 Viewer v0.3.1 之前的实现；根目录旧入口 `main.py`、legacy `plugins/`/`services/` 包和空兼容包 `utils/` 已在 DV-1008 中移除。
...
- 旧版 `HDF5 Viewer` 名称仅作为历史或迁移参考语境保留；当前目标包、入口和 UI 表面使用 `Data Viewer`。
