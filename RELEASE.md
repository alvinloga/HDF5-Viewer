...
- PyQt6/distribution licensing is resolved and documented;
...
Release evidence is generated only after the packaged functional smoke succeeds. The evidence generator records one checksum per platform archive, a dependency SBOM from the locked Python environment, third-party license notices, dependency-consistency status, and a path/credential leak scan. Until the PyQt distribution-license decision is documented as satisfied, `release-security-review.json` must report `release_status: blocked` and public publishing remains unavailable.
