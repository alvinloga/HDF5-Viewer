...
| `core/` | HDF5-centric loading/models/cache/event bus/slicer/async loader | cache, event-bus, string-slicer, and async-loader modules removed in DV-1008; remaining behavior replaced through DataSource/domain contracts one group at a time |
| `gui/` | shell, tabs, trees, views | legacy main window, sidebar package, secondary Search/Plugins panel, old activity-rail chrome modules, obsolete status/bottom/command chrome, and editor implementation modules removed in DV-1008; remaining GUI package shells are removed only after target shell/view parity checks |
