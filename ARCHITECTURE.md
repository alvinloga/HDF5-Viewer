...
The current `core/` and `gui/` packages are legacy migration inputs. The former legacy `plugins/` and `services/` packages and empty `utils/` compatibility package were removed in DV-1008 and are preserved only in history. New architecture belongs under `data_viewer/` unless a task explicitly creates a compatibility bridge.
